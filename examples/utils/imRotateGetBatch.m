...
%[ptsx,ptsy] = meshgrid(1:size(im,2),1:size(im,1));
%c=cat(2,ptsx',ptsy');
%pts=reshape(c,[],2);
%pts=[pts ones(size(pts,1),1)];
%trf=[cosd(theta1) -sind(theta1) 0; sind(theta1) cosd(theta1) 0; 0 0 1];
%C = bsxfun(@times,pts,trf);
...
%imo = imrotate(im,theta1,'bilinear','crop');

%tform = affine2d([cosd(theta1) -sind(theta1) 0; sind(theta1) cosd(theta1) 0; 0 0 1]);
%imo = imwarp(im,tform);
%[gcn,grn] = transformPointsForward(tform,pts(:,1), pts(:,2));
